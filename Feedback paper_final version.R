...
ggsave(file="D:/PhD Project/Feedback paper/Data and codes/Output data/Plots/Time series.jpeg",p,width=9,height=12)

#################################################################################################################
############################## Get LOVECLIM simulated global mean temperature change     ########################
#################################################################################################################
...
DO_Tsurf<-brick("D:/PhD Project/Feedback paper/Data and codes/Temperature data/LOVECLIM temperature.nc")
DO_Tsurf <- rasterToPoints(DO_Tsurf, spatial=TRUE)# Convert raster to SpatialPointsDataFrame
DO_Tsurf <- spTransform(DO_Tsurf, CRS(DO_Tsurf@proj4string@projargs))# reproject sp object
DO_Tsurf.all <- data.frame(lon=coordinates(DO_Tsurf)[,1],lat=coordinates(DO_Tsurf)[,2],DO_Tsurf@data)    # Assign coordinates to @data slot                     
age<-c(50000:29801)
colnames(DO_Tsurf.all)<-c("lon","lat",age)

DO_Tsurf_age_melt <- melt(DO_Tsurf.all,id.vars = c("lon","lat"))
colnames(DO_Tsurf_age_melt)<-c("lon","lat","age","T")

DO_Tsurf_age_melt$age<-as.numeric(as.character(DO_Tsurf_age_melt$age))
DO_Tsurf_age_melt$lon<-as.numeric(DO_Tsurf_age_melt$lon)
DO_Tsurf_age_melt$lat<-as.numeric(DO_Tsurf_age_melt$lat)
...
write.csv(DO_Tsurf_binned,"D:/PhD Project/Feedback paper/Data and codes/Temperature data/DO_Tsurf_binned.csv")

######################## Get weighted global mean temperature
DO_Tsurf_binned<-read.csv("D:/PhD Project/Feedback paper/Data and codes/Temperature data/DO_Tsurf_binned.csv",row.names = 1)
...
ggsave(file="D:/PhD Project/Feedback paper/Data and codes/Output data/Plots/Tglobal_w_mean.jpeg",p,width=8,height=5)
...
######################## Get land points
DO_Tsurf_land_binned<-DO_Tsurf_binned
DO_Tsurf_land_binned<-cbind.data.frame(DO_Tsurf_land_binned,wrld)
DO_Tsurf_land_binned<-DO_Tsurf_land_binned[!is.na(DO_Tsurf_land_binned$FIPS),]#get land points

#get weighted global mean land temperature
DO_Tsurf_land_mean<-DO_Tsurf_land_binned[,c("age","lat","lon","T")]
DO_Tsurf_land_mean$age<-as.factor(DO_Tsurf_land_mean$age)

Tland_w_mean<-data.frame()
for(i in 1:nlevels(DO_Tsurf_land_mean$age)){
  each<-DO_Tsurf_land_mean[which(DO_Tsurf_land_mean$age==levels(DO_Tsurf_land_mean$age)[i]),]
  each$w<-cos(each$lat*2*pi/360)
  Tw<-sum(each$T*each$w)/sum(each$w)
  value<-cbind.data.frame(levels(DO_Tsurf_land_mean$age)[i],Tw)
  colnames(value)<-c("age","Tw")
  Tland_w_mean<-rbind.data.frame(Tland_w_mean,value)
}

Tland_w_mean$Tw_30ka<-Tland_w_mean[which(Tland_w_mean$age==30000),"Tw"]
Tland_w_mean$Tw_anomaly<-Tland_w_mean$Tw-Tland_w_mean$Tw_30ka
Tland_w_mean$age<-as.numeric(Tland_w_mean$age)
...
###################### compare
identical(Tglobal_w_mean$age,Tland_w_mean$age)
identical(Tglobal_w_mean$age,Tocean_w_mean$age)

compareT<-cbind.data.frame(Tglobal_w_mean$age,Tglobal_w_mean$Tw_anomaly,Tland_w_mean$Tw_anomaly,Tocean_w_mean$Tw_anomaly)
colnames(compareT)<-c("age","Tglobal_anomaly","Tland_anomaly","Tocean_anomaly")

p<-ggplot()+
  geom_point(data=compareT,aes(age,Tland_anomaly),size=0.5)+geom_line(data=compareT,aes(age,Tland_anomaly),size=0.4)+ 
...
sum<-summary(lm(data=compareT,Tland_anomaly~Tocean_anomaly-1))
...
setwd("D:/PhD Project/Feedback paper/Data and codes/Output data/Plots/Map")
world <- map_data("world") 
